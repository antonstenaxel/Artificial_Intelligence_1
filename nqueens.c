...
#define MAXITER 1000
void hillClimbing() {
  int iter = 0, row, column, count, choice;
  int optimum = (nqueens-1)*nqueens/2;
  int *rows = malloc(sizeof(*rows));
  int *columns = malloc(sizeof(*columns));

  while (evaluateState() != optimum) {
    printf("iteration %d: evaluation=%d\n", iter++, evaluateState());
    if (iter == MAXITER) break;  /* give up */
    /* generate a (new) random state: for each queen do ...*/
    int lowestN = 999999, lowestc =0, lowestr =0, n = 99999;
    for (row=0; row < nqueens; row++) {
      int initColumn = queens[row];
      for(column =0; column< nqueens; column++){
      ...
      moveQueen(row, initColumn);
    }
    srand((unsigned int)time(NULL));

    choice = random() % (count+1);
    lowestr = rows[choice];
    lowestc = columns[choice];
     moveQueen(lowestr, lowestc);
  }
  if (iter < MAXITER) {
    printf ("Solved puzzle. ");
  }
